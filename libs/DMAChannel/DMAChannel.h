...
extern libcpp::function< void () >
	dma_channel_interrupt_handlers[DMA_NUM_CHANNELS];
...
public:
	...
	void attachInterrupt(void (*func)(void*), void* arg) {
		dma_channel_interrupt_handlers[channel].func = func;
		dma_channel_interrupt_handlers[channel].arg = arg;
		NVIC_ENABLE_IRQ(IRQ_DMA_CH0 + channel);
	}

	void detachInterrupt(void) {
		NVIC_DISABLE_IRQ(IRQ_DMA_CH0 + channel);
		dma_channel_interrupt_handlers[channel] = {};
	}
...
public:
	...
	// the entire transfer, and also optionally when half of the
	// transfer is completed.
	void attachInterrupt(libcpp::function< void() > const& handler) {
		dma_channel_interrupt_handlers[channel] = handler;
		NVIC_ENABLE_IRQ(IRQ_DMA_CH0 + channel);
	}
