...
libcpp::function< void () > dma_channel_interrupt_handlers[DMA_NUM_CHANNELS];

template< size_t Channel >
void invoke_dma_channel_interrupt_handler()
{
	auto&& handler = dma_channel_interrupt_handlers[Channel];
	if ( handler ) handler();
}
...
extern "C" {
	...
	void dma_ch0_isr()
	{
		invoke_dma_channel_interrupt_handler<0>();
	}

	void dma_ch1_isr()
	{
		invoke_dma_channel_interrupt_handler<1>();
	}

	void dma_ch2_isr()
	{
		invoke_dma_channel_interrupt_handler<2>();
	}

	void dma_ch3_isr()
	{
		invoke_dma_channel_interrupt_handler<3>();
	}
